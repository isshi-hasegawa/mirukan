...
- `docs/ideas/` は各 TODO に紐づく未確定の案、比較検討中の方針、設計メモを置く場所として扱い、独立したメモ置き場にはしない。
- `docs/ideas/` の各ファイルは、どの TODO の検討メモかが分かる状態に保ち、TODO から参照できるようにする。
- TODO だけで着手判断できる小さい話は `docs/ideas/` を作らず、方針比較や保留論点がある項目だけ `docs/ideas/` を使う。
- `docs/ideas/` にある案は明示的な依頼や合意なしに採用案を確定しない。
...
- 関連する TODO から参照されているアイデアや設計メモがある場合のみ `docs/ideas/` の該当ファイルも確認し、既存の論点や保留事項と矛盾しないように進める。
...
- 方針未確定の TODO について検討メモを追加する場合は、先に TODO 側に課題を置き、その TODO に紐づく `docs/ideas/` ファイルを追加する。
