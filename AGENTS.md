...
追加の見直しが必要なときだけ、節目ごとに self-review または `.agents/skills/adversarial-review/SKILL.md` を使う。
...
- adversarial-review は高コストなので、破壊的変更、広い影響範囲、複雑な境界条件、運用回帰の懸念、説明や検証だけでは確信しきれない変更に限って使う。
- 軽微な変更、局所変更、既存パターンへの単純追従では adversarial-review を呼ばず、self-review と必要な検証で完了させる。
- 批判的な観点で追加レビューしたい場合は adversarial-review を使ってよいが、対応可否の判断と実装修正は標準フロー側で行う。
