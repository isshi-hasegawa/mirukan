...
- 着手前に、依頼内容に関連する `docs/todo/` を確認する。
- `docs/todo/` は未着手または保留中の課題だけを置き、完了済み・不要・現状と乖離した項目は残さない。
- `docs/ideas/` は `docs/todo/` に紐づく検討メモとして扱い、必要な場合のみ確認する。
- TODO や idea は実装の強制指示ではなく、既存の課題認識・保留論点の確認資料として扱い、実際に着手する範囲はユーザー依頼を優先する。
- 作業中に TODO の前提が変わった場合は更新し、新しい未着手課題が見つかった場合のみ適切な TODO に追記する。
