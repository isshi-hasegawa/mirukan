...
- `main` で作業を始める場合は、まず `origin/main` との差分を確認し、`origin/main` が進んでいれば先に `pull` してから `AGENTS.md` や作業対象を確認する。
- コーディングエージェントを含むすべての作業者は、編集やコミットの前に現在のブランチを確認し、`main` にいる場合は新しい作業ブランチを作成してから作業する。
- 変更後は適切な単位でコミットする。
