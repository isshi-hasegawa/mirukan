...
- ユーザー価値として独立する未着手機能、運用改善、テスト改善は GitHub Issue で管理する。
- `docs/` は Issue 化前の短期メモや実装前調査メモが必要な場合にのみ使う。常設 backlog として増やさない。
- `docs/` に置くメモは実装の強制指示ではなく判断材料として扱い、着手範囲はユーザー依頼を優先する。
- Issue 化したメモ、完了済みのメモ、不要になったメモ、現状と乖離したメモは残さず削除する。
