...
- 作業開始時は `AGENTS.md` を正として確認し、あわせて `docs/README.md` を見て `docs/` 内の補助ドキュメント配置を把握する。
- ユーザー依頼に関連する TODO やアイデアがありそうな場合は、着手前に `docs/todo/` と `docs/ideas/` の該当ファイルを確認し、既存の方針・論点・保留事項と矛盾しないように進める。
- `docs/todo/` は「未着手の改善候補」を置く場所として扱い、今から着手する作業の直接の実装指示とはみなさない。実際に着手する範囲はユーザー依頼を優先する。
- `docs/ideas/` は「未確定の案・設計メモ」を置く場所として扱い、実装時は採用する案を勝手に確定させず、必要ならユーザーに確認する。
- 実装中に新しい TODO、後続で検討したい案、今回見送る論点が見つかった場合は、既存ファイルへ追記するか新規ファイルを追加して `docs/` に記録することを検討する。
- `docs/todo/` は未実施の項目だけを残し、完了した TODO は削除または別の適切なドキュメントへ移して陳腐化を防ぐ。
- 開発ルール、コマンド運用、確定済みの前提は `AGENTS.md` を正とし、`docs/` には補足情報・検討メモ・未確定事項を置く。
