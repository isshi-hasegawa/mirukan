...
1. 現在のブランチを確認する。
2. 現在のブランチが今回の作業を継続する適切な作業ブランチでない場合
   （例: `main`、無関係なブランチ、すでにマージ済みのローカルブランチ）、
   `main` に移動する。
3. `main` で `git fetch origin` を実行し、`origin/main` に差分があれば
   `git pull --ff-only origin main` で fast-forward のみで最新化する。
4. 調査・実装を問わず、最新の `main` を基準に `AGENTS.md` と作業対象を確認する。
5. 編集やコミットを伴う作業では、`main` を最新化した後にブランチ命名規則に従って
   作業ブランチを作成する。
