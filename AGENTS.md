...
- `docs/` の詳細運用は `docs/README.md` を正とする。
- 着手前に、依頼内容に関連する `docs/backlog/` を確認する。
- `docs/backlog/` は Issue 化前または保留中の論点、実装前の調査メモ、repo 固有の判断材料だけを置き、完了済み・不要・現状と乖離した項目は残さない。
- `docs/ideas/` は `docs/backlog/` に紐づく検討メモとして扱い、必要な場合のみ確認する。
- backlog や idea は実装の強制指示ではなく、既存の課題認識・保留論点の確認資料として扱い、実際に着手する範囲はユーザー依頼を優先する。
- 作業中に backlog の前提が変わった場合は更新し、新しい未整理の論点が見つかった場合のみ適切な backlog に追記する。
