---
name: cleanup
description: 作業完了後に使う。main に戻り、全ローカル branch と worktree を削除して main を最新化する。
---

# Cleanup

## いつ使うか

- 作業完了後に環境をきれいにしたいとき

## いつ使わないか

- 作業がまだ終わっていないとき
- 残しておきたい branch がある場合（削除前に確認する）

## 責務

- main に切り替える
- 全ローカル branch と worktree を削除する
- main を最新化する

## 正本

- repo 共通ルールは `AGENTS.md` を正本とする

## 判断ルール

- 削除前に未コミット変更と未 push commit の有無を確認する
- 未コミット変更がある場合は、`commit` / `stash` / `破棄` のどれにするかを先に確定する
- 未 push commit があり、今後参照する可能性があるなら push を検討する
- main にマージ済みの branch はスクワッシュマージのためハッシュが異なっても削除してよい
- 変更も commit も不要で、作業を完全に捨てるときだけ `reset --hard` と `clean -fd` を使う

## 手順

1. 全 branch と worktree を洗い出す
2. 未コミット変更と未 push commit を確認する
3. 必要に応じて commit / stash / 破棄を確定する
4. worktree を削除する
5. main に切り替えて、残りの全 branch を削除する
6. `git pull` で main を最新化する

## 停止条件

- 未コミット変更があるのに `commit` / `stash` / `破棄` の方針が決まっていない
- 未 push commit を消す判断が未確定
- 何かのプロセスが対象 worktree を使用中で、安全に削除できない

## 出力

- 削除したローカル branch の一覧
- 閉じた worktree の path 一覧
- 削除前に行った整理内容
- 実行した主要コマンド
- 未実施の確認や残リスク
