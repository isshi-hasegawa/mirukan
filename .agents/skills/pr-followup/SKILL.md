...
- review 指摘や CI 失敗を整理する
- PR の主題に関するユーザー所見を整理する
- ローカル修正、検証、コミット、必要なら push まで進める
- 修正後は self-review を行い、必要なら adversarial-review と pr-open へつなぐ
...
- review / CI だけでなく、既存 open PR の主題に沿うユーザー所見も継続作業として扱う
- review 指摘が主題なら unresolved thread を優先して確認する
- CI 失敗が主題なら失敗ジョブとログを優先して確認する
- 反論や補足だけで閉じる指摘は、無理にコード変更へ寄せない
- 対応済みの review thread は、返信より resolve を優先する
- AI review に対しては、説明が必要な例外を除き返信しない
...
11. 対応済み thread は resolve し、必要時のみ [`pr-open`](../pr-open/SKILL.md) や review reply に進む
...
- 実行した `vp` コマンド
- 追加した commit
- push / PR 更新 / thread resolve / review reply の有無
