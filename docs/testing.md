...
### Edge Functions の Deno テスト実行

- `supabase/functions/` 配下の Deno テストは
  `vp exec deno test --allow-env --node-modules-dir=none supabase/functions/`
  で実行する。
- `tmdb_test.ts` などが `Deno.env.get/set/delete` を使って
  テスト内で環境変数を差し替えるため、
  ローカル実行でも `--allow-env` が必要。
